...
const DEFAULT_NAVY_KEY = 'sk-navy-lZ4HVhr_FVvz9cmg1S4M5VjqxhknLWHPwezTNYssxlg';
...
async function startServer() {
  ...
  app.post('/api/chat', async (req, res) => {
    try {
      ...
      const effectiveKey = requestedKey || process.env.NAVY_API_KEY || DEFAULT_NAVY_KEY;

      // 1. If key is a Navy AI key (sk-navy-...), attempt Navy AI OpenAI-compatible endpoint
      if (effectiveKey.startsWith('sk-navy-')) {
        try {
          const navyMessages: Array<{ role: 'system' | 'user' | 'assistant'; content: string }> = [
          ...
          if (navyResponse.ok) {
            ...
            const errorBody = await navyResponse.text();
            console.warn('Navy AI returned status', navyResponse.status, errorBody, '- Falling back to Google Gemini');
          }
        } catch (navyErr) {
          console.warn('Navy AI connection failed, falling back to Google Gemini:', navyErr);
        }
      }

      // 2. Google Gemini fallback with multi-model resiliency (handles temporary 503 high demand)
      const geminiApiKey =
        (!effectiveKey.startsWith('sk-navy-') && effectiveKey) ||
      ...
      // Try preferred models in order to ensure high-demand 503 errors never break the user experience
      ...
      for (const modelName of candidateModels) {
        try {
          ...
          if (isRetryable) {
            console.warn(`Model ${modelName} returned temporary error: ${err.message}. Retrying with next model...`);
            continue;
          }
          break;
        }

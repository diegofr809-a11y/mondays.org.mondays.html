...
interface LucideGamesViewProps {
  games: Game[];
...
export const LucideGamesView: React.FC<LucideGamesViewProps> = ({
  games = [],
  onPlayGame,
  onOpenAddGame,
  onToggleFavorite,
  ...
  const [selectedCategory, setSelectedCategory] = useState<GameCategoryType>('all');
  ...
  // Sliced games for smooth rendering performance
    ...
    <div className="flex-1 h-screen overflow-y-auto px-4 sm:px-6 py-6 lucide-bg select-none">
      <div className="max-w-7xl mx-auto space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[var(--border-color)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full bg-[var(--badge-bg)] text-[var(--accent-color)] border border-[var(--border-color)] text-[11px] font-semibold">
                  {games.length} Games
                </span>
              </div>
              <p className="text-xs text-[var(--text-muted)]">
                Sandboxed HTML5 library synchronized with study.html games
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[var(--bg-surface)] hover:bg-[var(--bg-hover)] border border-[var(--border-color)] text-xs font-semibold text-[var(--text-main)] transition-all"
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[var(--accent-color)] hover:opacity-90 text-white text-xs font-semibold shadow-md transition-all"
            >
        ...
        <div className="pt-2">
          {visibleGames.length === 0 ? (
            <div className="text-center py-16 rounded-xl bg-[var(--bg-card)] border border-[var(--border-color)] space-y-2">
              ...
              <h3 className="text-sm font-semibold text-[var(--text-main)]">
                No games matched your criteria
              </h3>
              <p className="text-xs text-[var(--text-muted)]">
                Try searching for another title or clearing category filters.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {visibleGames.map((game) => (
                <div
                  key={game.id}
                  className="group bg-[var(--bg-card)] hover:bg-[var(--bg-hover)] border border-[var(--border-color)] hover:border-[var(--accent-color)] rounded-xl p-2 flex flex-col transition-all duration-200 shadow-sm hover:shadow-lg"
                >
                  {/* Thumbnail area */}
                  <div
                  ...
                  <div className="mt-2 flex-1 flex flex-col justify-between">
                    <div className="flex items-start justify-between gap-1">
                      <h4
                        className="text-xs font-semibold text-[var(--text-main)] truncate group-hover:text-[var(--accent-color)] transition-colors cursor-pointer"
                        onClick={() => onPlayGame(game)}
                        title={game.title}
                      >
                        {game.title}
                      ...
                      <button
                        ...
                        className="text-[var(--text-dim)] hover:text-amber-400 p-0.5"
                    ...
                    <div className="flex items-center justify-between text-[10px] text-[var(--text-muted)] mt-1.5 pt-1.5 border-t border-[var(--border-color)]">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          title="Stealth Popout (about:blank)"
                        >
                        ...
                        {game.isCustom && (
                          <button
                            ...
                            className="text-[var(--text-dim)] hover:text-red-400 p-0.5"
                            title="Delete"
                          >
          ...
          {/* Load More Button if filteredGames > displayLimit */}
          {filteredGames.length > displayLimit && (
            <div className="flex justify-center py-6">
              <button
                onClick={() => setDisplayLimit((prev) => prev + 72)}
                className="px-5 py-2 rounded-lg bg-[var(--bg-surface)] hover:bg-[var(--bg-hover)] border border-[var(--border-color)] text-xs font-semibold text-[var(--text-main)] shadow-sm transition-all"
              >

...
import { MessageSquare, Send, Hash, Users, Smile } from 'lucide-react';
...
export const LucideChatView: React.FC = () => {
  const [currentChannel, setCurrentChannel] = useState<'general' | 'gaming' | 'music'>('general');
  const [allMessages, setAllMessages] = useState<Record<string, ChatMessage[]>>(DEFAULT_MESSAGES);
    ...
    <div className="flex-1 h-screen flex lucide-bg select-none">
      ...
      <div className="w-48 bg-[#090712]/80 border-r border-[#1c172e] flex flex-col p-3 space-y-3">
        ...
        <div className="space-y-1">
          {(['general', 'gaming', 'music'] as const).map((channel) => (
            <button
              key={channel}

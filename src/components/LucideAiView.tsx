...
interface LucideAiViewProps {
  settings?: AppSettings;
}

export const LucideAiView: React.FC<LucideAiViewProps> = ({ settings }) => {
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    // Build chat history for Gemini multi-turn context (excluding the very first welcome message and any errors)
    const historyPayload = nextMessages
      .slice(1, -1) // exclude welcome greeting and latest user prompt
      .filter((m) => !m.isError)
      .slice(-10) // last 10 messages for memory
      .map((m) => ({

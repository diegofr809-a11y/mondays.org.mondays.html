...
export const AddGameModal: React.FC<AddGameModalProps> = ({
  ...
  const [category, setCategory] = useState<GameCategoryType>('arcade');
  ...
  const loadPreset = (t: string, c: GameCategoryType, u: string, d: string) => {
    setTitle(t);
    setCategory(c);
    setUrl(u);
    setDescription(d);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs select-none">
      <div className="bg-[#100d1e] border border-[#251f3b] rounded-xl max-w-md w-full p-5 space-y-3.5 text-xs shadow-2xl">
        ...
        {/* Quick test presets (Legal open-source web games) */}
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div>
              ...
              <select
                value={category}
                onChange={(e) => setCategory(e.target.value as GameCategoryType)}
